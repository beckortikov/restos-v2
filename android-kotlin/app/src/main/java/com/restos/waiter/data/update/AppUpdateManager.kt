...
data class UpdateCheck(
    val available: Boolean, // на кассе вообще есть APK
...
/**
 * LAN-автообновление приложения официанта: сверяем версию с кассой, качаем APK
 * по LAN и запускаем системную установку (в один тап — Android покажет
 * подтверждение). Полностью тихая установка потребовала бы Device Owner.
 */
@Singleton
class AppUpdateManager @Inject constructor(
    @ApplicationContext private val context: Context,
    private val api: WaiterAppApi,
) {
    val installedVersionName: String
        get() = runCatching {
    ...
    /** minSdk 26 → всегда есть canRequestPackageInstalls(). */
    ...
    /** Качает APK в cacheDir, репортит прогресс 0..1. Возвращает файл. */
    suspend fun download(onProgress: (Float) -> Unit): File = withContext(Dispatchers.IO) {
        val body = api.downloadApk()
        val total = body.contentLength()
        val file = File(context.cacheDir, "waiter-update.apk")
