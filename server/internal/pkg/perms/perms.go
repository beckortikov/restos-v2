...
var roleDefaults = map[string]map[string]bool{
	...
	// приложение :checkin). Устройство стоит без присмотра и активируется
	// один раз, поэтому прав у роли НЕТ ВООБЩЕ: украденный планшет не даёт
	// доступа ни к меню, ни к заказам, ни к деньгам. Отметки прихода/ухода
	// пойдут через собственные эндпоинты, которые проверяют роль сами.
	"checkin": {},
}
