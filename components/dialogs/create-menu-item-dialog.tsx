...
interface CreateMenuItemDialogProps {
  ...
  onSubmit: (menuItem: MenuItemForm) => void
}
...
// ─── Main Dialog ───────────────────────────────────────────────────────────
export function CreateMenuItemDialog({ open, onOpenChange, onSubmit }: CreateMenuItemDialogProps) {
        ...
        <div className="space-y-4">
          ...
          {!techCardsEnabled ? null : form.isPurchased ? (
            ...
            <div className="space-y-3 p-4 rounded-xl border border-blue-200 bg-blue-50/50">
              ...
              <p className="text-[10px] text-blue-600">Система автоматически создаст ингредиент на складе и привяжет к блюду. Приёмка через накладные.</p>
            </div>
          ) : (
            <>
