...
// один раз PIN'ом (BoardLayout). Статусы идут из кухонного приложения; табло
// только отражает и обновляется по SSE (RealtimeCacheBridge инвалидит ['orders']).
...
const COOK_DEFAULT_MIN = 8 // если у заказа нет expected_ready_at — целимся в 8 мин
...
// Прогресс готовки 0..1: прошло / ожидаемое. Никогда не 0 и не 1 — пустая и
// полная полоса читались бы как «ничего не делается» / «уже готово».
function cookProgress(o: Order, now: number): number {
  const start = new Date(o.createdAt).getTime()
  if (!Number.isFinite(start)) return 0.5
  const end = o.expectedReadyAt ? new Date(o.expectedReadyAt).getTime() : start + COOK_DEFAULT_MIN * 60_000
  if (end <= start) return 0.9
  return Math.min(0.97, Math.max(0.06, (now - start) / (end - start)))
}
...
function playChime() {
  ...
  } catch { /* автоплей заблокирован до взаимодействия — не критично */ }
}

export default function BoardPage() {
  const { data: orders = [] } = useQuery({
    queryKey: ['orders', 'board'],
    queryFn: () => fetchOrders({ from: startOfTodayISO() }),
  ...
  const cooking = useMemo(
    () => orders.filter(o => o.status === 'cooking').sort((a, b) => (a.createdAt || '').localeCompare(b.createdAt || '')),
    [orders],
  )
  const ready = useMemo(
    () => orders.filter(o => o.status === 'ready').sort((a, b) => (b.readyAt || '').localeCompare(a.readyAt || '')),
    [orders],
  )
  ...
  useEffect(() => {
    const cur = new Set(ready.map(o => o.orderNumber ?? 0))
    if (prevReady.current) {
      for (const n of cur) if (!prevReady.current.has(n)) { playChime(); break }
    }
      ...
      <section className="min-h-0 flex flex-col" style={{ borderRight: '1px solid rgba(255,255,255,0.07)', padding: 'clamp(16px,2vw,32px)' }}>
        ...
        <div className="flex-1 min-h-0 overflow-hidden" style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(10px,1.4vw,20px)' }}>
          {cooking.length === 0 ? (
            ...
            const p = cookProgress(o, now)
            return (
              <div key={o.id} style={{ background: '#141a22', borderRadius: 16, padding: 'clamp(10px,1.1vw,18px) clamp(14px,1.4vw,22px)' }}>
