...
import { LayoutGrid, Users, Plus, CalendarPlus, Combine, X, Clock, Check, Ban } from 'lucide-react'
import { toast } from 'sonner'
import { useAuth } from '@/lib/auth-store'
import { useOrderData } from '@/components/order/use-order-data'
import { createReservation, fetchReservationForTable, updateReservationStatus, mergeTables, unmergeTables } from '@/lib/queries'
...
export default function PosV2Tables() {
  ...
  const MODES: [Mode, string][] = [['order', 'Заказы'], ['reserve', 'Бронь'], ['merge', 'Объединить']]
    ...
    <div className="flex flex-col h-full w-full overflow-hidden">
      ...
      {mode !== 'order' && (
        <div className="shrink-0" style={{ padding: '0.5rem var(--pv-pad-x) 0', color: 'var(--pv-text-3)', fontSize: 'var(--pv-ctl)' }}>
          {mode === 'reserve' ? 'Нажмите свободный стол, чтобы забронировать.' : mergePrimary ? `Выбран стол ${mergePrimary.number} — нажмите второй для объединения. Объединённый стол — нажать для разъединения.` : 'Нажмите два стола для объединения (или объединённый — для разъединения).'}
        </div>
      )}
