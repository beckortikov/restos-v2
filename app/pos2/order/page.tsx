...
export default function PosV2Order() {
  ...
  useEffect(() => {
    ...
    // Незакрытый заказ прежнего типа не должен молча пропадать с экрана: «К
    // оплате» создаёт заказ СРАЗУ (до подтверждения оплаты) — если кассир
    // передумал/ошибся типом и закрыл панель оплаты крестиком, активная
    // группа остаётся open, но после переключения типа выпадает из вида (её
    // не покажет очередь другого типа). Данные не теряются — заказ жив,
    // найдётся через «Заказы» — но кассир об этом не знает и рискует набрать
    // те же блюда заново, оставив дубль-заказ висеть в старой очереди.
    if (activeGroup && activeGroup.status !== 'done' && activeGroup.status !== 'cancelled') {
      // activeGroup.type — тип САМОГО заказа (где он реально остаётся), не
      // orderType состояния: эффект уже сработал ПОСЛЕ смены orderType на
      // новый, так что новый тип тут был бы неверной подсказкой («останется
      // в Собой», хотя реально висит в Зале).
      const total = activeGroup.totalWithService ?? activeGroup.total ?? 0
      toast.warning(`Заказ №${activeGroup.orderNumber ?? ''} (${ORDER_TYPE_LABELS[activeGroup.type]}) остался открытым — ${formatCurrency(total)}`, {
        description: 'Найдёте его через «Заказы»',
        duration: 6000,
      })
    }
