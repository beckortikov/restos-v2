...
export default function WaiterAppPage() {
  const [info, setInfo] = useState<WaiterAppInfo | null>(null)
  ...
  const loadInfo = useCallback(async () => {
    try {
      const res = await fetch(getBaseURL() + '/api/v1/waiter-app', { headers: authHeaders() })
  ...
  const downloadUrl = lanIp ? `http://${lanIp}:${PORT}/download/waiter.apk` : ''
  ...
  async function handleUpload(file: File) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(await res.text())
      setInfo(await res.json())
      toast.success('APK загружен — официанты могут обновляться по QR')
    } catch (e) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 p-4 sm:p-6">
      <div>
        <h1 className="text-2xl font-bold">Приложение официанта</h1>
        <p className="text-sm text-muted-foreground">
          Раздача APK по локальной WiFi: официант сканирует QR телефоном и скачивает приложение.
        </p>
      </div>
          ...
          <CardContent className="space-y-4 p-4">
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="text-center">
                <div className="font-semibold">Скачать на телефон официанта</div>
            ...
            <div className="space-y-1 border-t border-border pt-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">Как обновить приложение у официантов:</p>
              <ol className="list-decimal space-y-0.5 pl-4">
                <li>Соберите новый APK и нажмите «Загрузить APK» здесь.</li>
                <li>Официант сканирует QR камерой телефона.</li>
      ...
      {!info?.available && !loading && (
        <div className="flex items-start gap-2 rounded-lg bg-muted/40 px-3 py-2.5 text-sm text-muted-foreground">
          ...
          <span>После загрузки APK здесь появится QR-код для раздачи официантам.</span>
        </div>
      )}
    </div>
  )
