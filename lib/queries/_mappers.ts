...
export function mapRestaurantRow(r: Record<string, any>): Restaurant {
  return {
    ...
    lateGraceMinutes: r.late_grace_minutes ?? 5,
    lateFineFixed: String(r.late_fine_fixed ?? 0),
    lateFinePerMinute: String(r.late_fine_per_minute ?? 0),
    lateFineMax: String(r.late_fine_max ?? 0),
    shiftRoundingMinutes: r.shift_rounding_minutes ?? 0,
...
export function mapUserRow(r: Record<string, any>): User {
  return {
    ...
    payType: r.pay_type === 'daily' ? 'daily' : r.pay_type === 'hourly' ? 'hourly' : 'monthly',
    dailyRate: r.daily_rate != null ? Number(r.daily_rate) || 0 : 0,
    hourlyRate: r.hourly_rate != null ? Number(r.hourly_rate) || 0 : 0,
