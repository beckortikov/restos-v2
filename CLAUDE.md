...
Живые кассы (Electron + embedded-postgres на Windows) обновляются авто-апдейтом, часто **перепрыгивая** версии (напр. v3.16.130 → v3.16.148). Каждое правило ниже — след реального инцидента, когда касса вставала после апдейта. Нарушение = ресторан без кассы. **Проверять на каждом релизе, который трогает миграции или старт бэка.**

1. **Номера миграций строго монотонны по времени добавления.** Новая миграция получает номер **выше** любой уже задеплоенной. Никогда не вставлять миграцию с номером ниже того, что уже накатан на кассах — strict-goose падает `missing migrations before current version` и уводит бэк в рестарт-луп. *(Инцидент 22.07.2026: `057_shift_op_account` добавлена в v3.16.132 — позже, чем `059` в v3.16.130.)* Подстраховка `goose.WithAllowMissing()` в [conn.go](server/internal/db/conn.go) включена, но дисциплину номеров держим — это не повод нумеровать как попало.
2. **Миграции идемпотентны — всегда.** `CREATE TABLE/INDEX IF NOT EXISTS`, `ADD COLUMN IF NOT EXISTS`, `DROP ... IF EXISTS`. Тогда повторный прогон, self-heal и применение вне очереди безопасны (no-op на здоровой БД).
3. **pgdata НИКОГДА не удаляется автоматически.** Ни при какой ошибке старта (auth, занятый порт, упавшая миграция). Авто-wipe стёр базу ресторану. *(Инцидент 14.07.2026: миграция 035 упала → авто-wipe → потеря данных.)* Реальный password-mismatch чинится только вручную и только с бэкапом — см. комментарий в [main.go](server/cmd/restos-server/main.go).
4. **После успешного старта embedded-PG любой фатальный выход сначала зовёт `sup.Stop()`** (хелпер `fatalStopPG` в [main.go](server/cmd/restos-server/main.go)). `log.Fatal`/`os.Exit` пропускают `defer`'ы → PG сиротеет на порту 54330 → каждый рестарт sidecar'а падает с `process already listening on port 54330`, касса down навсегда. *(Инцидент 22.07.2026.)*
5. **Ошибка старта восстановима, не «навсегда».** Electron освобождает порты (`ensurePortFree` в `app.on('ready')` **и** на рестарт-пути `goProc.on('exit')` — [desktop/main.js](desktop/main.js)). Помни для диагностики: **reboot сам по себе логическую ошибку старта (миграцию) не лечит** — нужен исправленный бинарь или откат на версию до бага.
6. **Тестировать апдейт с ПЕРЕПРЫГОМ версий, а не только чистую установку.** Свежая установка накатывает миграции по порядку и прячет out-of-order баги — они видны только на «прыжке». Прогон: накатить схему до версии N (напр. воспроизвести `goose_db_version` кассы), затем прогнать бинарь версии N+K против неё.
7. **Критичную drift-опасную схему до-гарантировать на каждом старте** через `EnsureCriticalSchema` ([selfheal.go](server/internal/db/selfheal.go)) — независимо от `goose_db_version`. Кассы уходят в рассинхрон (goose says «применено», а колонок нет). Новую критичную колонку/таблицу, от которой зависит закрытие смены/заказа, дублируем идемпотентным DDL в self-heal.
