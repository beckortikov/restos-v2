...
10a. **Бинари PG (`PGBinariesDir`, `pg-bin/<ver>`) и `RuntimePath` (`pg-runtime`) — РАЗНЫЕ каталоги. Не сливать.** Библиотека embedded-postgres на каждом `Start()` делает `os.RemoveAll(RuntimePath)`; если бинари лежат там же (как было до этого) — их стирает и распаковывает 133 МБ заново КАЖДЫЙ старт (~15-20с + скан антивирусом). Разведены → распаковка один раз, обычный старт ~250мс. При падении старта [supervisor.go](server/internal/pgsupervisor/supervisor.go) `wipeBinaries()` сбрасывает ТОЛЬКО `pg-bin` (не `pgdata`!) и делает одну чистую распаковку — самолечение битых/сожранных антивирусом бинарей. `pgdata` не трогается никогда. *(ускорение холодного старта.)*
...
### Перед локальным запуском Electron

`desktop/resources/` — сборочный артефакт, в git его нет (CI собирает всё сам,
см. `.github/workflows/desktop-release.yml`). На чистом клоне сайдкара там
просто не будет, поэтому сначала:

```bash
cd server && make build-sidecar
```

Не подкладывать туда старый бинарь «чтобы запустилось»: расхождение версий
бэка и кода — источник ложных диагнозов. Ровно так и вышло с закоммиченным
`restos-server` — он был на два месяца старше кода, не содержал разведения
`pg-bin`/`pg-runtime`, и локальный старт занимал 12 секунд вместо 0,15.
